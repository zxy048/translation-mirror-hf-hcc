...
FIGURES = {
    ...
    "**Fig. 2. Independent identification and functional conservation of translation "
    "co-expression programs across HF and HCC.**": {
        "num": "Fig. 2",
        "files": [os.path.join(FIGURES_DIR, "Figure2_CrossDisease_Conservation.png")],
        "caption": (
            "Independent identification and functional conservation of translation co-expression programs across HF and HCC. "
    ...
    "**Fig. 3. Cross-disease comparison of pathway effect sizes.**": {
        "num": "Fig. 3",
    ...
    "**Fig. 4. TGS prognostic validation and hub gene cross-disease directionality.**": {
        "num": "Fig. 4",
    ...
    "**Fig. S1. Cross-disease directional consistency": {
        "num": "Fig. S1",
    ...
    "**Fig. S2. RPL39 single-gene expression": {
        "num": "Fig. S2",
    ...
    "**Fig. S3. Independence of hub genes": {
        "num": "Fig. S3",
    ...
    "**Fig. 5. Upstream regulator analysis of the translation co-expression program.**": {
        "num": "Fig. 5",
        "files": [
    ...
    "**Fig. 6. Cross-disease mirror regulation framework": {
        "num": "Fig. 6",
    ...
    "**Fig. S4. Soft threshold selection": {
        "num": "Fig. S4",
        "files": [
    ...
    "**Fig. S5. Disease specificity analyses": {
        "num": "Fig. S5",
        "files": [os.path.join(FIGURES_DIR, "Figure_S5_Validation_SameOrgan_Controls.png")],
        "caption": (
            "Disease specificity analyses of cross-disease mirror perturbation. "
            "Cross-disease Spearman correlation of pathway-level Cohen's d effect sizes (x-axis: HCC, TCGA-LIHC) "
            "for 33 translation-related pathways (red) and 50 other pathways (gray). "
            "(A) HF discovery cohort (GSE57338): ρ = −0.598, 24/33 mirror. "
            "(B) Cardiac same-organ control (GSE141910, HCM vs. NF): ρ = −0.036 (p = 0.84) — "
            "no mirror perturbation, excluding a generic cardiac disease effect. "
            "(C) Liver same-organ control (GSE89377, cirrhosis vs. normal): ρ = +0.402 (p = 0.021) — "
            "translation pathways positively correlated with HCC, consistent with pre-malignant translational activation. "
            "(D) Independent HF cohort (GSE116250): ρ = +0.249, 14/33 mirror — "
            "overall cross-disease trend retained (all-pathway ρ = −0.283, p = 0.011); "
            "translation subset shows cohort-dependent heterogeneity. "
            "Mirror perturbation was absent in both same-organ comparisons, supporting disease-context specificity."
        ),
        "width": 15.5,
    },
}
...
def build_document():
    ...
    # Skip "Regular Paper" label, author block, ARTICLE INFO, ABSTRACT header
    # until we hit the actual abstract text
    abstract_lines = []
    in_pre_abstract = True
    while i < len(lines):
        s = lines[i].strip()
        # Skip everything until after "A B S T R A C T"
        if in_pre_abstract:
            if re.match(r'^A\s+B\s+S\s+T\s+R\s+A\s+C\s+T\s*$', s):
                in_pre_abstract = False
                i += 1
                continue
            i += 1
            continue
        # After "A B S T R A C T", collect abstract text until the first `---`
        if s == '---':
            i += 1
            break
        if s:
    ...
    # Skip blank lines until next section
    while i < len(lines) and lines[i].strip() == '':
        i += 1
    ...
    run = p_tag.add_run("Regular Paper")
    run.font.size = Pt(11)
    run.font.name = 'Times New Roman'
    ...
    run = p_auth.add_run("[Author 1]ᵃ, [Author 2]ᵇ, [Corresponding Author]ᵃ,*")
    run.font.size = Pt(11)
    run.font.name = 'Times New Roman'
    ...
    run = p_aff1.add_run("ᵃ [Department, Institution, City, Country]")
    run.font.size = Pt(9)
    run.font.name = 'Times New Roman'

    p_aff2 = doc.add_paragraph()
    p_aff2.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p_aff2.add_run("ᵇ [Department, Institution, City, Country]")
    run.font.size = Pt(9)
    run.font.name = 'Times New Roman'
    ...
    p_ai.alignment = WD_ALIGN_PARAGRAPH.LEFT
    run = p_ai.add_run("A R T I C L E   I N F O")
    run.font.bold = True
    run.font.size = Pt(11)
    run.font.name = 'Times New Roman'

    p_kw_label = doc.add_paragraph()
    run = p_kw_label.add_run("Keywords:")
    run.font.italic = True
    run.font.size = Pt(11)
    run.font.name = 'Times New Roman'
    ...
    run = p_kw.add_run("Translation; Heart failure; Hepatocellular carcinoma; WGCNA; ssGSEA; ATF4; Integrated stress response; MYC; Mirror regulation")
    run.font.size = Pt(11)
    run.font.name = 'Times New Roman'
    ...
    run = p_abs_head.add_run("A B S T R A C T")
    run.font.bold = True
    run.font.size = Pt(11)
    run.font.name = 'Times New Roman'
    ...
    if abstract_lines:
        ...
        p_abs.paragraph_format.first_line_indent = Cm(0.75)
    ...
    for level, heading, sec_lines in sections:
        ...
        # Insert Table 1 after Methods 2.1
        if heading.startswith("2.2") and not table1_inserted:
            add_table1(doc)
        ...
        for line in sec_lines:
            ...
            # Skip ASCII art blocks and markdown image syntax
